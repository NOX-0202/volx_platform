...
function Category() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (category.id) {
                req = await api.put(`/products/categories?uuid=${category.id}`, category);
            } else {
                req = await api.post("/products/categories", category);
            }

            console.log(req)

            if (req.data.success) router.push("/dashboard/produtos/categorias");
        } catch (error) {
            console.log(error);
        }
    }
    ...
    useEffect(() => {
        (async () => {
            ...
            if (router.query.category_id && router.query.category_id !== 'adicionar') {
                ...
                const get_category = await api.get(`/products/categories?id=${router.query.category_id}`)
        ...
        {loading ? <div className="text-center my-4"><div className="spinner-border text-primary text-center" role="status" /> </div> :
            <div className="container">
                <div className="d-flex align-items-center my-4 gap-3 ">
                    <a href='/dashboard/produtos/categorias' >
