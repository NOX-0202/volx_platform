  ...
  const deleteCategory = async (id_category) => {
    try {
      ...
      if (confirm("Deseja realmente excluir esta categoria?")) {
        const delete_req = await api.delete(`/posts/categories?id=${id_category}`);
        if (delete_req.success) getCategories();
      }
    } catch (error) {
      console.log(error);
    }
  ...
  return <main>
    ...
    <div className="container">
      <div className="d-flex justify-content-between my-4 ">
        <h1>Categorias</h1>
        <a href='/dashboard/posts/categorias/adicionar' >
            ...
            <tbody>
              {categories.length > 0 && categories.map(category => <tr key={category.id}>
                ...
                <td className='d-flex gap-3'>
                  <a href={`/dashboard/posts/categorias/${category.id}`}>
